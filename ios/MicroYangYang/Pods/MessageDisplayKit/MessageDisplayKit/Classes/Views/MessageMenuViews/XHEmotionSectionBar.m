...
- (void)reloadData {
    ...
    float w = self.bounds.size.width/self.emotionManagers.count;
    for (XHEmotionManager *emotionManager in self.emotionManagers) {
        ...
        [sectionButton setImage:[UIImage imageNamed:@"Expression_1.png"] forState:UIControlStateNormal];
        
        CGRect sectionButtonFrame = sectionButton.frame;
        sectionButtonFrame.origin.x = index * w;
        sectionButtonFrame.size.width = w;
